...
const { Consumer, Provider } = React.createContext({ mode: RenderMode.Passthrough });
...
export const FrondRender: React.SFC<RenderRootProps> = ({ children, mode }) => <Provider value={{ mode }}>{children}</Provider>
...
function getRenderedComponent(Component: any, props: any) {
  return ReactDOM.renderToString(
    <FrondableDecendant.Provider value={true}><Component {...props} /></FrondableDecendant.Provider>
  );
}
...
class Frond extends React.Component<FrondProps> {
  ...
  render() {
    return (
      <FrondableDecendant.Consumer>{(decendant) => (
        <Consumer>
          {({ mode }) => {
            const componentProps = this.props.props;
            const Component = this.props.component;
            const { container, className, name, disableSSR } = this.props.options;
            ...
            if (mode === RenderMode.Passthrough || decendant) {
              return Container({
                ...
                'data-component': name !== null ? name : this.props.component.name,
              })
            }
...
export function Frondable(options?: FrondableOptions) {
  return (Component: any) =>  {
    return ((props: any) => <Frond options={options || {}} props={props} component={Component} />) as any;
  }
}
